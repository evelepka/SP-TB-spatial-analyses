"""Two-column metro table — adults (≥15 years) only.

Identical to 49e_table_metro.py but:
  • Population denominator = IBGE 2022 pop aged ≥15 per census sector
  • TB cases filtered to age_tb ≥ 15
...
Outputs:
  /tmp/table_metro_adult.png
  /tmp/table_metro_adult.docx
"""
...
sec = sec22.merge(pop_df[["CD_SETOR","pop_adult","pop_total"]], on="CD_SETOR", how="left")
...
sec_res = sec_res.rename(columns={"pop_adult": "pop"})
...
cohort_meta["year"] = pd.to_datetime(
    cohort_meta["notification_date"], errors="coerce").dt.year
...
cohort_geo["age_tb"] = cohort_geo["sinan_clean"].map(age_map)
cohort_geo = cohort_geo[cohort_geo["year"].between(2013, 2024)].copy()
cohort_geo = cohort_geo[cohort_geo["age_tb"] >= 15].copy()   # ← adult filter
TOTAL_CASES_STATE = len(cohort_geo)
...
# ── UNIT AGGREGATION + SELECTION (state-wide) ─────────────────────────────────
...
# ── RESTRICT TO GSP+BS ────────────────────────────────────────────────────────
g1_ids = {uid for uid in selected_ids if unit_region.get(uid) == "GSP_BS"}
g2_ids = {uid for uid in unit_df["unit_id"]
          if unit_region.get(uid) == "GSP_BS" and uid not in selected_ids}

metro_cohort_geo = cohort_geo[cohort_geo["region"] == "GSP_BS"].copy()
TOTAL_CASES_METRO = len(metro_cohort_geo)

print(f"  GSP+BS selected units: {len(g1_ids)}  |  non-selected: {len(g2_ids)}")
print(f"  Total metro adult cases: {TOTAL_CASES_METRO:,}  |  metro adult pop: {POP_GSP_BS/1e6:.2f}M")
...
cohort = metro_cohort_geo.merge(clin, on="sinan_clean", how="left")

g1_cases = cohort[cohort["unit_id"].isin(g1_ids)]
g2_cases = cohort[cohort["region"] == "GSP_BS"][
    ~cohort["unit_id"].isin(selected_ids)]

# ── STATS ─────────────────────────────────────────────────────────────────────
def unit_stats(uid_set):
    ...
    return {
        ...
        "cases":        int(cases),
        "pct_cases":    cases / TOTAL_CASES_STATE * 100,
...
s1 = unit_stats(g1_ids)
ns_pop   = POP_GSP_BS        - s1["pop"]
ns_cases = TOTAL_CASES_METRO - s1["cases"]
ns_units = unit_df[(unit_df["unit_id"].map(unit_region) == "GSP_BS") &
                   (~unit_df["unit_id"].isin(selected_ids))]
s2 = {
    "n_units":      len(ns_units),
    "pop":          int(ns_pop),
    "pct_pop":      ns_pop / TOTAL_POP * 100,
    "cases":        int(ns_cases),
    "pct_cases":    ns_cases / TOTAL_CASES_STATE * 100,
    "rate":         ns_cases / (ns_pop * 12) * 1e5 if ns_pop > 0 else 0,
    "med_pop":      int(ns_units["pop"].median()) if len(ns_units) else 0,
    "med_cases_yr": ns_units["n_cases"].median() / 5 if len(ns_units) else 0,
}

for tag, s in [("GSP+BS selected", s1), ("GSP+BS non-selected", s2)]:
    print(f"  {tag:25s}: {s['n_units']:>4} units | pop {s['pop']/1e6:.2f}M "
          f"({s['pct_pop']:.1f}%) | {s['cases']:,} cases ({s['pct_cases']:.1f}%) "
          f"| rate {s['rate']:.0f}/100k")
...
ROWS = [
    ("POPULATION & TB BURDEN  (adults ≥15 years)",        "",                              "",                             True),
    ("Number of geographic units",                         fn(s1["n_units"]),               fn(s2["n_units"]),              False),
    ("Total adult population (≥15 years)",                 fn(s1["pop"]),                   fn(s2["pop"]),                  False),
    ("% of SP state adult population",                     fp(s1["pct_pop"]),               fp(s2["pct_pop"]),              False),
    ("Adult TB cases 2013–2024 (N)",                       fn(s1["cases"]),                 fn(s2["cases"]),                False),
    ("% of SP state adult TB cases",                       fp(s1["pct_cases"]),             fp(s2["pct_cases"]),            False),
    ("TB incidence rate (per 100,000 adults/yr)",          fr(s1["rate"]),                  fr(s2["rate"]),                 False),
    ("Median unit adult population",                       fn(s1["med_pop"]),               fn(s2["med_pop"]),              False),
    ("Median adult cases/yr per unit",                     f"{s1['med_cases_yr']:.1f}",     f"{s2['med_cases_yr']:.1f}",   False),

    ("PATIENT CHARACTERISTICS",                            "",                              "",                             True),
    ("HIV positive (% of HIV-tested)",                     fp(c1["hiv_pos"]),               fp(c2["hiv_pos"]),              False),
    ("Pulmonary TB (%)",                                   fp(c1["pulm"]),                  fp(c2["pulm"]),                 False),

    ("DISCOVERY ROUTE",                                    "",                              "",                             True),
    ("Outpatient care — patient demand (%)",               fp(c1["outpt"]),                 fp(c2["outpt"]),                False),
    ("Emergency/urgency presentation (%)",                 fp(c1["er"]),                    fp(c2["er"]),                   False),
    ("Diagnosed during hospitalisation (%)",               fp(c1["hosp_dx"]),               fp(c2["hosp_dx"]),              False),
    ("Active case finding (ACF) (%)",                      fp(c1["acf"]),                   fp(c2["acf"]),                  False),
    ("Contact investigation (%)",                          fp(c1["contact"]),               fp(c2["contact"]),              False),

    ("TREATMENT OUTCOMES  (% of known-outcome cases)",     "",                              "",                             True),
    ("Treatment success / cure (%)",                       fp(c1["cure"]),                  fp(c2["cure"]),                 False),
    ("Lost to follow-up (%)",                              fp(c1["ltfu"]),                  fp(c2["ltfu"]),                 False),
    ("Death from TB (%)",                                  fp(c1["tb_death"]),              fp(c2["tb_death"]),             False),
    ("Hospitalised during treatment (%)",                  fp(c1["hosp_tx"]),               fp(c2["hosp_tx"]),              False),
]

FOOTNOTE = (
    "SINAN notifications 2013–2024 geocoded to IBGE CNEFE 2022  ·  Adults ≥15 years only  ·  "
    ...
    "Hotspot selection: ≥10 adult cases/5yr, state-wide 20% adult population window, FCU ≥5,000 pop  ·  "
    "% pop/cases = relative to all SP state adult residents and all geocoded adult cases  ·  "
    "HIV+: % of HIV-tested  ·  outcomes exclude transfers and diagnostic changes  ·  "
    f"{POP_GSP_BS/1e6:.1f}M adult residents  ·  {TOTAL_CASES_METRO:,} geocoded adult cases"
)
...
ROW_H = 0.32; SEC_H = 0.36; HDR_H = 0.52; TITLE_H = 0.40; FOOT_H = 0.32
...
def fill(x0, x1, y0, y1, c, z=1):
    ax.add_patch(patches.Rectangle((x0, y0), x1-x0, y1-y0,
                                   linewidth=0, facecolor=c, zorder=z))
...
ax.text(FW/2, FH - TITLE_H/2,
        "TB targeting — São Paulo urban agglomeration  ·  Adults ≥15 years  ·  2013–2024",
...
fill(X[0], X[1], y_hdr2, y_top, "#f0f4f8")
fill(X[1], X[2], y_hdr2, y_top, "#1a5c3a")
fill(X[2], X[3], y_hdr2, y_top, "#2d6a4f")

ax.text(xc(1), (y_top + y_hdr2) / 2, "Selected\nhotspots",
        ha="center", va="center", fontsize=10.5, fontweight="bold",
        color="white", multialignment="center", zorder=5)
ax.text(xc(2), (y_top + y_hdr2) / 2, "Non-selected\nareas",
        ha="center", va="center", fontsize=10.5, fontweight="bold",
        color="white", multialignment="center", zorder=5)
...
for row in ROWS:
    ...
    if is_sec:
        ...
        fill(X[1], X[2], y0, y_cur, ROW_BG[ci][0])
        fill(X[2], X[3], y0, y_cur, ROW_BG[ci][1])
        ax.text(PAD+0.10, (y_cur+y0)/2, label,
                ha="left", va="center", fontsize=9.5, color="#1a202c", zorder=5)
        for j, val in enumerate([v1, v2]):
...
vline(X[1], y_cur, FH - TITLE_H, "#4a5568", lw=0.8)
vline(X[2], y_cur, FH - TITLE_H, "#b0bec5", lw=0.5)

ax.text(PAD, y_cur - 0.07, FOOTNOTE,
        ha="left", va="top", fontsize=6.8, color="#718096", style="italic")

plt.subplots_adjust(left=0, right=1, top=1, bottom=0)
plt.savefig("/tmp/table_metro_adult.png", dpi=200, facecolor="white")
plt.close()
print("Saved /tmp/table_metro_adult.png")
...
r = title_p.add_run(
    "Characteristics of TB targeting areas\n"
    "São Paulo urban agglomeration · Adults ≥15 years · 2013–2024")
...
cell_text(r0.cells[1], "Selected hotspots",
          bold=True, size=11, color=(255,255,255), bg="#1a5c3a")
cell_text(r0.cells[2], "Non-selected areas",
          bold=True, size=11, color=(255,255,255), bg="#2d6a4f")

SEC_BG_HEX = "2c5282"
ROW_BG_DOCX = [
    ["f7fafc", "e6f4ec", "d4edda"],
    ["edf2f7", "c3e6cb", "b8ddc0"],
]
data_i = 0
for ri, row in enumerate(ROWS):
    ...
    if is_sec:
        tbl_row.cells[0].merge(tbl_row.cells[2])
        ...
        cell_text(tbl_row.cells[0], label, size=10, bg=bgs[0],
                  align=WD_ALIGN_PARAGRAPH.LEFT)
        cell_text(tbl_row.cells[1], v1, bold=True, size=10, bg=bgs[1])
        cell_text(tbl_row.cells[2], v2, bold=True, size=10, bg=bgs[2])
        data_i += 1

fn_p = doc.add_paragraph()
fn_p.paragraph_format.space_before = Pt(6)
r = fn_p.add_run(FOOTNOTE)
r.italic = True; r.font.size = Pt(7.5)
r.font.color.rgb = RGBColor(113, 128, 150)

doc.save("/tmp/table_metro_adult.docx")
print("Saved /tmp/table_metro_adult.docx")

"""SP state targeting figures — Lorenz curve, map, stability, characteristics table.

Four publication-quality output files:
  /tmp/fig1_lorenz_sp_state.png    — Lorenz curve (high → low)
  /tmp/fig2_map_sp_state.png       — Map of selected hotspot units
  /tmp/fig3_stability_sp_state.png — Year-to-year stability
  /tmp/fig4_table_sp_state.png     — Characteristics comparison table
"""
...
print(f"  Pop {TOTAL_POP/1e6:.1f}M | Cases {TOTAL_CASES:,}")
...
sec_w_tmp = sec_w.merge(cohort_all.groupby("CD_SETOR").size().rename("n_cs").reset_index(),
                        on="CD_SETOR", how="left")
sec_w_tmp["n_cs"] = sec_w_tmp["n_cs"].fillna(0)
...
# Recompute selection directly from unit_df (avoids mg_X ID mismatch from script 48)
# Eligible: ≥ MIN_CASES pooled 5yr.  Select by rate until 20% of SP state pop.
...
# Year-specific case counts per unit
yr_unit = cohort_all.dropna(subset=["unit_id"]).groupby(["unit_id","year"]).size().reset_index(name="n_yr")

def select_for_year(yr):
    """Return the set of unit_ids selected in a given year, restricted to the
    eligible pool (units with ≥ MIN_CASES pooled over all 5 years).
    This mirrors the pooled-selection methodology applied year-by-year."""
    yc = yr_unit[yr_unit["year"] == yr][["unit_id","n_yr"]]
    # base pool: same eligible set as the pooled analysis
...
yr_selections = {yr: select_for_year(yr) for yr in range(2020, 2025)}

# Jaccard by year pair
...
# Stability score: how many years was each unit in the top-20% selection?
all_sel_ever = set.union(*yr_selections.values())
stability = {}
for uid in all_sel_ever:
    stability[uid] = sum(1 for yr in range(2020,2025) if uid in yr_selections[yr])
...
# Cases% captured per year by the FIXED pooled-selection set
fixed_pct = {}
for yr in range(2020, 2025):
    yc = yr_unit[yr_unit["year"]==yr][["unit_id","n_yr"]]
    total_yr = yc["n_yr"].sum()
    in_sel   = yc[yc["unit_id"].isin(selected_ids)]["n_yr"].sum()
    fixed_pct[yr] = in_sel / total_yr * 100 if total_yr > 0 else 0

optimal_pct = {}
for yr in range(2020, 2025):
    yc = yr_unit[yr_unit["year"]==yr][["unit_id","n_yr"]]
    total_yr = yc["n_yr"].sum()
    in_opt   = yc[yc["unit_id"].isin(yr_selections[yr])]["n_yr"].sum()
    optimal_pct[yr] = in_opt / total_yr * 100 if total_yr > 0 else 0
...
# Use cases mapped to units as Lorenz denominator (so the curve reaches 100%)
# Some geocoded cases fall in sectors excluded from sec_res (non-residential / pop<100)
...
all_u["cum_cases"] = all_u["n_cases"].cumsum() / CASES_MAPPED * 100   # normalise to mapped

# For the annotation: report % of ALL geocoded cases (not just mapped)
idx_20       = np.searchsorted(all_u["cum_pop"].values, POP_WIN*100)
sel_cases_20 = all_u["n_cases"].cumsum().iloc[min(idx_20, len(all_u)-1)]
cases_at_20_mapped  = sel_cases_20 / CASES_MAPPED * 100    # % of mapped (for curve dot)
# Annotation/subtitle report the ACTUAL selected hotspots (eligible units, >=MIN_CASES) —
# matches the map and the characteristics tables (300 areas, 20.1% pop -> 39.2% cases).
# (sel_cases_20 above includes tiny ineligible high-rate units and overstates the share.)
cases_at_20_all     = sel_df["n_cases"].sum() / TOTAL_CASES * 100

x_lor = np.concatenate([[0], all_u["cum_pop"].values / 100])
y_lor = np.concatenate([[0], all_u["cum_cases"].values / 100])
# High-to-low ranking: curve bows ABOVE diagonal (concentrated areas at left).
# Concentration index = 2×AUC − 1  (positive when concentrated, 0 if equal)
...
ax.annotate(
    f"Top 20% of population\n→ {cases_at_20_all:.1f}% of all geocoded cases",
    xy=(POP_WIN*100, cases_at_20_mapped), xytext=(28, cases_at_20_mapped - 11),
    fontsize=10, color="#c0392b",
    arrowprops=dict(arrowstyle="->", color="#c0392b", lw=1.2),
)
ax.set_xlabel("Cumulative % of SP state population\n(bairros ranked highest → lowest TB rate)", fontsize=11)
ax.set_ylabel("Cumulative % of TB cases (2020–2024)", fontsize=11)
ax.set_title(f"TB case concentration — São Paulo state\n"
             f"Bairro/FCU-level analysis  ·  {len(all_u):,} geographic units  ·  "
             f"{TOTAL_POP/1e6:.0f}M residents",
             fontsize=12, fontweight="bold", pad=10)
ax.set_xlim(0,100); ax.set_ylim(0,100)
ax.legend(fontsize=10, loc="upper left")
ax.grid(alpha=0.25)
plt.tight_layout()
plt.savefig("/tmp/fig1_lorenz_sp_state.png", dpi=180, bbox_inches="tight", facecolor="white")
plt.close()
print("  Saved /tmp/fig1_lorenz_sp_state.png")
...
cbar.set_label("TB incidence rate\n(per 100,000 · 5-year pooled)", fontsize=9.5)
...
ax.set_title(f"TB hotspot areas — São Paulo state  ·  2020–2024\n"
             f"{len(sel_geom):,} units  ·  {TOTAL_POP*POP_WIN/1e6:.1f}M people (20% of state pop)  ·  "
             f"{cases_at_20_all:.1f}% of TB cases",
             fontsize=12, fontweight="bold", pad=12)
ax.set_xlabel("Longitude", fontsize=9); ax.set_ylabel("Latitude", fontsize=9)
ax.tick_params(labelsize=8); ax.set_aspect("equal")
plt.tight_layout()
plt.savefig("/tmp/fig2_map_sp_state.png", dpi=180, bbox_inches="tight", facecolor="white")
plt.close()
print("  Saved /tmp/fig2_map_sp_state.png")

# ══════════════════════════════════════════════════════════════════════════════
# FIGURE 3: Stability
# Two panels:
#   Left  — Jaccard index for each consecutive year-pair (bar chart)
#   Right — Map coloured by stability score (years in top-20% out of 5)
# ══════════════════════════════════════════════════════════════════════════════
print("\nFigure 3: Stability...")

# Stability map: dissolve ever-selected sectors, colour by stability score
...
# ── Panel A: Jaccard bars ──────────────────────────────────────────────────────
ax = axes[0]
pair_labels  = ["2020→2021","2021→2022","2022→2023","2023→2024"]
...
ax.set_title("A)  Year-to-year Jaccard stability\n"
             "(overlap of top-20%-pop selections)", fontsize=11, fontweight="bold")
ax.legend(fontsize=10); ax.grid(axis="y", alpha=0.3)
ax.tick_params(axis="x", labelsize=9, rotation=15)
for spine in ["top","right"]: ax.spines[spine].set_visible(False)

# Annotation explaining Jaccard
ax.text(0.5, -0.22,
...
# ── Panel B: Stability-score map ───────────────────────────────────────────────
ax = axes[1]
mun_bg.plot(ax=ax, color="#efefef", edgecolor="#c8c8c8", linewidth=0.3)

# Color by stability score 1–5
...
# Custom legend for stability scores
score_labels = {
...
score_colors = [stab_cmap(i/4) for i in range(5)][::-1]   # 5→1

# Count units per stability level
...
ax.set_title("B)  Stability score — years each area was in top-20% selection\n"
             f"(2020–2024,  {len(stab_geom):,} areas ever selected)",
             fontsize=11, fontweight="bold")
ax.set_xlabel("Longitude", fontsize=9); ax.set_ylabel("Latitude", fontsize=9)
ax.tick_params(labelsize=8); ax.set_aspect("equal")

fig3.suptitle("Year-to-year stability of TB hotspot targeting — São Paulo state",
              fontsize=13, fontweight="bold", y=1.01)
plt.tight_layout()
plt.savefig("/tmp/fig3_stability_sp_state.png", dpi=180, bbox_inches="tight", facecolor="white")
plt.close()
print("  Saved /tmp/fig3_stability_sp_state.png")
...
rows = [
    ...
    ("Total population",              [s_fcu["pop"],       s_oth["pop"],       s_nos["pop"],       s_tot["pop"]]),
    ("% of SP state population",      [s_fcu["pct_pop"],   s_oth["pct_pop"],   s_nos["pct_pop"],   100.0]),
    ("TB cases 2020–2024 (N)",        [s_fcu["cases"],     s_oth["cases"],     s_nos["cases"],     s_tot["cases"]]),
    ("% of SP state TB cases",        [s_fcu["pct_cases"], s_oth["pct_cases"], s_nos["pct_cases"], 100.0]),
    ("TB incidence rate (per 100k·yr)",[s_fcu["rate"],     s_oth["rate"],      s_nos["rate"],      s_tot["rate"]]),
    ("Median unit population",        [s_fcu["med_pop"],   s_oth["med_pop"],   s_nos["med_pop"],   s_tot["med_pop"]]),
]
...
ax4.set_title("Characteristics of TB targeting areas — São Paulo state\n"
              "Min 10 cases / 5 yr  ·  FCU ≥ 5,000 pop  ·  Min unit pop 2,000",
              fontsize=12, fontweight="bold", pad=16, y=0.98)
fig4.text(0.01, 0.01,
          f"SINAN notifications 2020–2024 geocoded to IBGE CNEFE 2022  ·  "
          f"{TOTAL_POP/1e6:.1f}M residents  ·  {TOTAL_CASES:,} cases",
          fontsize=8, color="#666")

plt.tight_layout()
plt.savefig("/tmp/fig4_table_sp_state.png", dpi=180, bbox_inches="tight", facecolor="white")
plt.close()
print("  Saved /tmp/fig4_table_sp_state.png")

print("\n✓ All outputs:")
for f in ["/tmp/fig1_lorenz_sp_state.png", "/tmp/fig2_map_sp_state.png",
          "/tmp/fig3_stability_sp_state.png", "/tmp/fig4_table_sp_state.png"]:
    print(f"  {f}")

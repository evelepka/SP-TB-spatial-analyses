"""Two-column characteristics table restricted to the metropolitan corridor:
  (1) GSP + Baixada Santista — Selected hotspots
  (2) GSP + Baixada Santista — Non-selected areas

Percentages are relative to GSP+BS totals (not all of SP state).
Selection criteria are identical to the state-wide analysis (state 20% pop window).

Outputs:
  /tmp/table_metro.png
  /tmp/table_metro.docx
"""
...
TOTAL_CASES_STATE = len(cohort_geo)   # all geocoded SP state cases 2020-2024
...
# ── UNIT AGGREGATION + SELECTION (state-wide, same as 49d) ───────────────────
...
print(f"  Total metro cases: {TOTAL_CASES_METRO:,}  |  metro pop: {POP_GSP_BS/1e6:.2f}M")
...
# Non-selected: residual to guarantee 100%
...
ROWS = [
    # (label, G1, G2, is_section)
    ("POPULATION & TB BURDEN",                          "",                              "",                             True),
    ("Number of geographic units",                      fn(s1["n_units"]),               fn(s2["n_units"]),              False),
    ("Total population",                                fn(s1["pop"]),                   fn(s2["pop"]),                  False),
    ("% of SP state population",                        fp(s1["pct_pop"]),               fp(s2["pct_pop"]),              False),
    ("TB cases 2020–2024 (N)",                          fn(s1["cases"]),                 fn(s2["cases"]),                False),
    ("% of SP state TB cases",                          fp(s1["pct_cases"]),             fp(s2["pct_cases"]),            False),
    ("TB incidence rate (per 100,000/yr)",              fr(s1["rate"]),                  fr(s2["rate"]),                 False),
    ("Median unit population",                          fn(s1["med_pop"]),               fn(s2["med_pop"]),              False),
    ("Median cases/yr per unit",                        f"{s1['med_cases_yr']:.1f}",     f"{s2['med_cases_yr']:.1f}",   False),
...
FOOTNOTE = (
    ...
    "Hotspot selection: ≥10 cases/5yr, state-wide 20% population window, FCU ≥5,000 pop  ·  "
    "% pop/cases = relative to all SP state (43.8M residents, all geocoded cases)  ·  "
    "HIV+: % of HIV-tested  ·  outcomes exclude transfers and diagnostic changes  ·  "
    f"{POP_GSP_BS/1e6:.1f}M residents  ·  {TOTAL_CASES_METRO:,} geocoded cases"
)
...
PAD  = 0.25
C0_W = 5.8    # label column
C1_W = 2.8    # selected hotspots
C2_W = 2.8    # non-selected areas
GAP  = 0.00
...
ROW_BG = [
    ["#e6f4ec", "#d4edda"],   # odd rows
    ["#c3e6cb", "#b8ddc0"],   # even rows
]
SEC_BG = "#2c5282"
...
ax.text(FW/2, FH - TITLE_H/2,
        "Characteristics of TB targeting areas — São Paulo urban agglomeration  ·  2020–2024",
        ha="center", va="center", fontsize=12, fontweight="bold",
        color="#1a202c", zorder=5)

# Column headers (single-level, no super-header)
y_top  = FH - TITLE_H
y_hdr2 = y_top - HDR_H
...
plt.savefig("/tmp/table_metro.png", dpi=200, facecolor="white")
plt.close()
print("Saved /tmp/table_metro.png")
...
for sec in doc.sections:
    ...
    sec.page_width    = Cm(21.0); sec.page_height  = Cm(29.7)  # A4 portrait

title_p = doc.add_paragraph()
title_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
r = title_p.add_run(
    "Characteristics of TB targeting areas\n"
    "São Paulo urban agglomeration · 2020–2024")
r.bold = True; r.font.size = Pt(13)
r.font.color.rgb = RGBColor(26, 32, 44)
title_p.paragraph_format.space_after = Pt(10)

# Table: 3 cols (label + 2 groups), header row + data rows
...
doc.save("/tmp/table_metro.docx")
print("Saved /tmp/table_metro.docx")

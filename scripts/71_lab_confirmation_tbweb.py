"""Laboratory (bacteriological) confirmation as a TB-programme fragility indicator,
sub-municipal, from TBweb (`lab_confirmed`). Equity question: are the most vulnerable /
highest-burden areas less laboratory-confirmed (more clinical-only diagnosis)?
Confirmation = % of cases laboratory-confirmed. Window 2020-2024 (comparable to the
molecular-coverage analysis, script 70).

Output: /tmp/fig_lab_confirmation.png
"""
...
print("Loading cohort + lab_confirmed...")
def lc(p,raw=False):
...
m=pd.read_csv(f"{SP}/cohort_with_spatial.csv",usecols=["sinan_clean","notification_date","age_tb","lab_confirmed","tx_seq"],low_memory=False,dtype={"sinan_clean":str})
...
for c in ["year","age_tb","lab_confirmed"]: co[c]=co["sinan_clean"].map(m[c])
...
co["conf"]=pd.to_numeric(co["lab_confirmed"],errors="coerce")
co["known"]=co["conf"].notna()

def cov(df): return df["conf"].sum()/df["known"].sum()*100 if df["known"].sum() else np.nan
print(f"\nStatewide laboratory confirmation: all 2013-24 = {cov(co[co['known']]):.1f}%")
for y in range(2013,2025):
    s=co[(co['year']==y)&co['known']]; print(f"  {y}: {cov(s):4.1f}%")
REC=co[co["year"].between(2020,2024)&co["known"]].copy()
print(f"Recent 2020-2024 confirmation = {cov(REC):.1f}%  (n={len(REC):,})")
...
print("\nLaboratory confirmation by vulnerability decile (1=least,10=most vulnerable):")
print(dr.round(1).to_string())

uagg=REC.groupby("unit_id").agg(conf=("conf","sum"),known=("known","sum")).reset_index()
uagg["cov"]=np.where(uagg["known"]>=10,uagg["conf"]/uagg["known"]*100,np.nan)
ginc=co.groupby("unit_id").size(); upop=sec.groupby("unit_id")["pop_adult"].sum()
...
print(f"\nLab confirmation in incidence hotspots {cov(REC[REC['unit_id'].isin(HS_inc)]):.1f}% vs rest {cov(REC[~REC['unit_id'].isin(HS_inc)]):.1f}%")
...
x=dr.index.values
axA.plot(x,dr["cov"],"o-",color="#1f6f8b",lw=2.6,ms=8)
axA.set_xlabel("Vulnerability decile (1=least → 10=most vulnerable)"); axA.set_ylabel("Laboratory confirmation, %")
axA.set_title("A)  Laboratory confirmation by vulnerability decile (2020–2024)\nare the most vulnerable areas less lab-confirmed?",fontsize=11.5,fontweight="bold")
...
d2.plot(column="cov",ax=axB,cmap="RdYlGn",vmin=vmin,vmax=vmax,edgecolor="white",lw=0.05,legend=True,legend_kwds={"shrink":0.55,"label":"Lab confirmation % (red=low=fragile)"})
axB.set_xlim(*ZOOM["xlim"]); axB.set_ylim(*ZOOM["ylim"])
axB.set_title("B)  Laboratory confirmation by unit — Greater SP + Baixada\n(2020–2024; units with ≥10 cases)",fontsize=11.5,fontweight="bold")
axB.set_xlabel("Longitude"); axB.set_ylabel("Latitude")
fig.suptitle("TB laboratory (bacteriological) confirmation — programme fragility, SP adults",fontsize=13,fontweight="bold")
plt.tight_layout(); plt.savefig("/tmp/fig_lab_confirmation.png",dpi=150,bbox_inches="tight"); plt.close()
print("\nSaved /tmp/fig_lab_confirmation.png")

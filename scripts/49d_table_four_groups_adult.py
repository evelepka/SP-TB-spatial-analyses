"""Four-group characteristics table — adults (≥15 years) only.

Identical to 49d_table_four_groups.py but:
  • Population denominator = IBGE 2022 pop aged ≥15 per census sector
  • TB cases filtered to age_tb ≥ 15

Groups:
  (1) São Paulo urban agglomeration — Selected hotspots
  (2) São Paulo urban agglomeration — Non-selected areas
  (3) Non-metropolitan SP state     — Selected hotspots
  (4) Non-metropolitan SP state     — Non-selected areas

Outputs:
  /tmp/table_four_groups_adult.png
  /tmp/table_four_groups_adult.docx
"""
...
print(f"  Adult TB cases 2013-2024: {TOTAL_CASES:,}  |  adult pop: {TOTAL_POP/1e6:.1f}M")
...
cases_gsp_bs   = cohort[cohort["region"] == "GSP_BS"].shape[0]
...
def nosel_stats(region_tag, sel_uid_set):
    ...
    region_cases = cases_gsp_bs if region_tag == "GSP_BS" else cases_interior
...
s1 = unit_stats(g1_ids, POP_GSP_BS,   cases_gsp_bs)
...
ROWS = [
    ...
    ("Total adult population (≥15 yr)", fn(s1["pop"]),       fn(s2["pop"]),        fn(s3["pop"]),        fn(s4["pop"]),        False),
    ("% of SP state adult population",  fp(s1["pct_pop"]),   fp(s2["pct_pop"]),    fp(s3["pct_pop"]),    fp(s4["pct_pop"]),    False),
    ("Adult TB cases 2013–2024 (N)",   fn(s1["cases"]),      fn(s2["cases"]),      fn(s3["cases"]),      fn(s4["cases"]),      False),
    ("% of SP state adult TB cases",    fp(s1["pct_cases"]),  fp(s2["pct_cases"]),  fp(s3["pct_cases"]),  fp(s4["pct_cases"]),  False),
    ("TB rate (per 100,000 adults/yr)", fr(s1["rate"]),       fr(s2["rate"]),       fr(s3["rate"]),       fr(s4["rate"]),       False),
    ("Median unit adult population",   fn(s1["med_pop"]),    fn(s2["med_pop"]),    fn(s3["med_pop"]),    fn(s4["med_pop"]),    False),
    ("Median adult cases/yr per unit", f"{s1['med_cases_yr']:.1f}", f"{s2['med_cases_yr']:.1f}",
...
FOOTNOTE = (
    ...
    "Hotspot selection: ≥10 adult cases/5yr, 20% adult population window, FCU ≥5,000 pop  ·  "
    "% of SP state = relative to all adult residents and all geocoded adult cases  ·  "
...
HDR_COL = ["#1a5c3a","#2d6a4f","#744210","#92400e"]
ROW_BG = [
    ["#e6f4ec","#d4edda","#fef3c7","#fde68a"],
    ["#c3e6cb","#b8ddc0","#fde68a","#fcd34d"],
]
SEC_BG = "#2c5282"
...
ax.text(FW/2, FH - TITLE_H/2,
        "Characteristics of TB targeting areas by region — São Paulo state  ·  Adults ≥15 years  ·  2013–2024",
        ha="center", va="center", fontsize=12, fontweight="bold", color="#1a202c", zorder=5)

y_super = FH - TITLE_H
y_hdr2  = y_super - HDR_H

fill(X[1],X[3], y_hdr2, y_super, "#1a4731")
fill(X[3],X[5], y_hdr2, y_super, "#5c3a00")
...
plt.savefig("/tmp/table_four_groups_adult.png", dpi=200, facecolor="white")
plt.close()
print("Saved /tmp/table_four_groups_adult.png")
...
for j, (txt, bg) in enumerate([
    ...
    cell_text(r1.cells[j+1], txt, bold=True, size=10, color=(255,255,255), bg=bg)
...
doc.save("/tmp/table_four_groups_adult.docx")
print("Saved /tmp/table_four_groups_adult.docx")

...
# OR table for the supplement (m2, all fixed effects)
ors <- data.frame(term = rownames(co)[-1], OR = or[-1], lo = lo[-1], hi = hi[-1])
write.csv(ors, "/tmp/ltfu_glmm_ors.csv", row.names = FALSE)
cat("ORs -> /tmp/ltfu_glmm_ors.csv\n")
